...
compileCondition cteId (E.PropertyEquals propertyName property) =
  ...
  , [ [qm| :propertyName{cteId} |] := propertyName
    ...
    , [qm| :propertyTextValue{cteId} |] :=
      (fromProperty property :: Maybe T.Text)
    , [qm| :propertyUTCTimeValue{cteId} |] :=
      (fromProperty property :: Maybe UTCTime)
    ])
-- TODO(#516): E.ConditionAnd is not interpreted in Sqlite.Compiler.compileCondition
compileCondition _ (E.ConditionAnd _) = undefined
-- TODO: E.PropertyGreater is not interpreted in Sqlite.Compiler.compileCondition
...
compileCteChain name (E.Filter condition input) =
  ...
  , ( [qms| t{filterCteId}(eid) as (
    ...
    inputQuery)
  where
    (inputCteId, inputQuery) = compileCteChain name input
...
compileCteChain name (E.SortBy propertyName order input) =
  ...
  , ( [qms| t{sortByCteId}(eid) as (
    ...
    , [[qm| :propertyName{sortByCteId} |] := propertyName]) <>
    inputQuery)
  where
    (inputCteId, inputQuery) = compileCteChain name input
...
compileSelector name selector = header <> body <> footer cteId
  where
    (cteId, body) = compileCteChain name selector
